...
export class CohortVisualizationProvider {
  ...
  async render(visData, visParams, status) {
    ...
    _.map(customerData, (d, day) => {
      /* Get all the uniq customerIds as the total */
      const customerIds = _.chain(d)
        .map('_id')
        .uniq()
        .value();
    ...
    /* restaurant */
    showTable(this.vis.params.mapColors, 'd', this.container, data, valueFn, 'restaurant');

    /* menu */
    showTable(this.vis.params.mapColors, 'd', this.container1, data1, valueFn, 'menu');

    /* quote */
    showTable(this.vis.params.mapColors, 'd', this.container2, data2, valueFn, 'quote');
  }
...
async function getCustomersFromEs(params) {
  return await axios({
    method: 'post',
    url: '../api/customer_visit_cohort/query/visit',
    data: { ...params },
