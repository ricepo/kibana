...
export class RestaurnatMonthlyReportVisualizationProvider {
  ...
  async render(visData, visParams, status) {
    ...
    const restaurants = _.chain(esData)
      .filter(
        r => !r.suspended && moment(r.createdAt).isAfter(from) && moment(r.createdAt).isBefore(to)
      )
      .map(r => {
    ...
    if (!newRests.length) {
      ...
      this.container.innerHTML = `There are no new drivers between ${moment(registeredFrom).format(
        'YYYY-MM-DD'
      )} and ${moment(registeredTo).format('YYYY-MM-DD')}`;
