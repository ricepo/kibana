...
export class DriverWeeklyReportVisualizationProvider {
  ...
  async render(visData, visParams, status) {
    ...
    while (moment(currentWeek).isBefore(to)) {
      ...
      const lastWeek = moment(currentWeek)
        .subtract(7, 'days')
        .format('YY-MM-DD');
      ...
      const newDriver = _.filter(
        accounts,
        a =>
          moment(a.createdAt).isBefore(currentWeek) &&
          moment(a.createdAt).isAfter(
            moment(currentWeek)
              .subtract(14, 'days')
              .toDate()
          ) &&
