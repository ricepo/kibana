...
export function showTable(element, data) {
  ...
  _.forEach(orders, v => {
    ...
    const driverShifts = _.chain(shifts)
      ...
      .sortBy(['start'])
      .value();
    const duration = _.isFinite(_.sumBy(driverJobs, 'duration') / 60)
      ? _.sumBy(driverJobs, 'duration') / 60
      : 0;
    ...
    const { hours = 0, noCallnoShow = 0, incompleteShift = 0, shiftHours = 0 } = getInterval(
      driverJobs,
      driverShifts
    );
